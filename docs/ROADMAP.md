...
| ---: | --- | --- | --- | --- | --- |
| 0 | Database integrity and Shopping persistence foundation | Partial; owner-authenticated administration surface implemented locally and unpublished save approved but not executed | Execute preservation in a future session, then separately gated publication/export/upgrade/verification | Medium | Save not yet executed; production schema remains unverified; book-to-collection foreign key deferred |
