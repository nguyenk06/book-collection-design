...
- At new-project intake and before activating a new large milestone, Quatre retrieves the authoritative Codex account usage meter when available and records its source and observation time. The Product Owner need not provide a screenshot or percentage. A Product Owner `!<five-hour>:<longer-period>` reading remains a fallback or additional observation when the authoritative meter is unavailable.
- A second reading immediately before `!run` is required only when the intake reading is stale, a reset or other material usage occurred, the slice is large/high-risk, or the remaining high estimate may no longer fit above the floor. Do not request duplicate readings for a small or normal continuation in the same window with ample margin.
- Treat local session-log, overlay, or cache-derived readings as advisory because they may lag the account meter. Do not use an advisory reading in place of an available authoritative reading at a project-start or large-milestone gate.
...
- For the five-hour window, use **10% remaining as the clean-stop target** and **5% remaining as the hard lower boundary**. Plan new work against the remaining high estimate plus the 10% target. Stop earlier when the next safe checkpoint cannot fit; never invent work to consume the 5–10% band or intentionally plan below 5%.
- Preserve **15% remaining in the longer-period/weekly window** by default. Only explicit Product Owner direction may override that weekly floor, normally for a bounded finish near its scheduled reset. A weekly exception does not override the five-hour hard boundary, authority, production, safety, or Site gates.
- Planning-class guides are: diagnostic/status only **35%**; small implementation **50%**; normal implementation **70%**; migration, release, or other high-risk work **85%**. These help size fresh work but are not universal vetoes. Split work into independently resumable safe units when the remaining high estimate plus the 10% five-hour target does not fit or would cross the weekly floor.
...
- During execution, request another reading only for a large/high-risk checkpoint, a reset/window transition, material scope or estimate growth, another project's intervening consumption, or credible approach to either floor. A practical five-hour risk trigger is when the current reading may be within roughly ten points of the remaining high estimate plus the 10% clean-stop target. Capture an ending reading when readily available, but do not interrupt or block an otherwise complete small/normal run merely to obtain it. Model, reasoning, tools, context, and task complexity can change actual usage; Designer calibrates ranges from evidence rather than treating percentages as fixed work units.
- Retrieving usage is read-only. Never redeem an available usage-reset credit without explicit Product Owner authorization; a reset does not broaden project, production, or Site authority.

Before activating `!run`, Quatre reviews both usage periods and uses Relena when product/release-boundary sizing is needed. The active goal identifies dependencies, genuine approval gates, production-risk boundaries, shared hotspots, likely blockers, and clean stopping cost. Keep useful approved work moving toward the five-hour 10% clean-stop target while preserving the weekly 15% floor. Reduce or checkpoint work when the remaining high estimate cannot preserve either applicable boundary or no safe checkpoint fits.

After `!run`, Kira owns practical sequencing within the approved goal. Kira may investigate, implement, test, remediate ordinary defects, converge, validate, and prepare the required packet without repeated Product Owner approval. The run is completion-oriented: an ordinary in-scope lint error, test failure, build failure, or reversible local defect is evidence for the next authorized remediation attempt, not by itself a reason to stop or request a new brief. Continue through up to three distinct substantive approaches for the same problem when the accepted scope permits them, recording attempts internally and returning to Quatre only after completion or a genuine stopping boundary. A blocked stream is preserved and reported only when it needs direction or cannot advance; Kira may then move to another independently eligible, non-conflicting stream already authorized inside the same goal. Productively use available five-hour capacity by continuing the highest-value eligible CYOA work; never invent work or generate activity solely to burn usage. Enter `WAITING FOR RESET` when the five-hour clean-stop target or hard boundary, the weekly floor, or the absence of a safe authorized unit requires it. Production publication, schema/data writes, destructive or difficult-to-reverse operations, rollback/restore, and other recorded gates always retain their separate explicit approvals.
