...
The approved administration surface is implemented and validated locally without a database change and preserved as unpublished Site Version 19. Before the existing upgrade service can run, the protected POST requires owner authorization, same-origin validation, JSON content type, a dedicated action header, and the exact deliberate confirmation value. The UI presents schema status first and disables export/upgrade on partial or unexpected state. Saving Version 19 did not publish or perform a production request, export, migration, or verification.
