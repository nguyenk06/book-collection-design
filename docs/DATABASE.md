...
This bridge is implemented, locally validated, preserved as Site Version 18, and retained behind published Version 19. Its archive includes migrations `0000` through `0003` and excludes `0004`. Disposable tests cover authorization/pre-upgrade readability, repeated reconciliation, partial-state recovery, duplicate invocation, preservation, and export. Reconciliation inspects each object, re-inspects after duplicate column errors, changes the CYOA target only when null, and completes only after schema/foreign-key/target/preservation checks. The runtime duplicate guard is not distributed. Gates 2–4 later verified the bounded production status/export/activation sequence; broader application publication and live behavior remain unverified.

Version 18 was published successfully through Sites and is retained behind published Version 19. Gate 2 subsequently used the Version 19 owner-authenticated surface to observe the expected pre-upgrade baseline with zero foreign-key issues and create a privately retained validated bridge export. The export contained 215 Books, 4 Collections, 0 Businesses, 0 Purchases, and 17 cover references with no duplicate identifiers or broken references. It is not a D1 snapshot, excludes R2 bytes, and authorizes no schema write.

Earlier owner-only schema-status/export attempts aborted before application responses because the Engineer lacked the Product Owner-controlled owner session. Gate 2 later established the supported same-origin path without transferring session material and completed the bounded read/export preflight.
...
The approved administration surface is implemented and validated locally without a database change and is now published as Site Version 19. Before the existing upgrade service can run, the protected POST requires owner authorization, same-origin validation, JSON content type, a dedicated action header, and the exact deliberate confirmation value. The UI presents schema status first and disables export/upgrade on partial or unexpected state. Version 19 publication reached deployment status `succeeded`, but application rendering was not independently observed and no production request, export, migration, or verification occurred.
...
During the 2026-08-20 local theme QA run, the active local D1 environment returned `table books has no column named stable_id` for `/api/books`. A later read-only diagnosis confirmed that this Miniflare D1 contains only base migrations `0000`–`0003`: `books.added_at`, `books.stable_id`, Shopping/identifier tables, and the stable-ID index/triggers are absent, and no Drizzle ledger is present. The configured Drizzle journal contains only `0000`–`0003`; tests explicitly apply the separate ordered `shopping-migrations/` files `0004`–`0006`, but the local runtime does not apply them automatically. No source, configuration, schema, ledger, or data change occurred. This is local-environment drift, not production evidence. Any remediation must first back up the exact local database, prove ordered `0004` → `0005` → `0006` on a disposable clone, verify preservation and application behavior, and receive separate authority before changing the active local database.

That bounded local remediation completed on 2026-08-21. Engineer created and verified a private byte-for-byte backup, proved unchanged `0004` → `0005` → `0006` once on a disposable clone, then applied the same sequence transactionally once to the exact quiescent active local D1. The empty local dataset retained 0 Books, 3 Collections, 0 Businesses, 0 Purchases, 0 identifiers, and 0 cover references; identity/value hashes matched, CYOA target price became 600 cents, stable-ID nulls/duplicates and foreign-key issues were zero, and expected schema/index/trigger enforcement was present. Current-source validation passed 101/101 tests, scoped lint with one pre-existing warning, and production build. Source/configuration, Sites, and production remained unchanged; no restore or additional write was authorized.
