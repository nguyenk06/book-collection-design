...
- **Active owner:** `EXTERNAL / WAIT` — wait for the five-hour reset, provide one fresh usage reading, then send Sei III `!brief` when the operational minimum is restored. A separate later `!run` remains required.
- **Blocking owner:** `NONE`; `WAITING FOR RESET` is a normal resumable capacity state, not a project blocker. Process quiescence remains unknown and must be resolved only through the brief-authorized supported read-only gate before execution commands begin.
...
Product Owner supplied `!14:87`: **14% five-hour** and **87% longer-period**. The window is below the provisional 15% automatic stopping floor, so the prepared Sei III continuation is `WAITING FOR RESET` and must not enter `!brief` or `!run`. Its estimate remains 5/9/15 points with a 30% operational minimum after reset. The residual capacity is reserved for essential checkpoint, reporting, or recovery only.
