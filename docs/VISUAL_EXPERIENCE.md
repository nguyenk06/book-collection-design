...
- **Confirmed validation remediation — saved / release identity ambiguous:** checkpoint `80e4c61` remains retired as historical evidence. Replacement checkpoint `f15ea81`, based on exact Version 20, implements the five bounded corrections and is saved as Version 21. Diagnostic focused tests passed 17/17 and 3/3, then all remaining release gates passed. The sole publication invocation returned no deployment identity; a one-query read-only identity verification is authorized and prepared before hands-on validation. See the [Version 20 remediation plan](VERSION20_CONFIRMED_FAILURE_REMEDIATION_PLAN.md).
