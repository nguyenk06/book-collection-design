...
The simple sum is **62 / 91 / 128** points. Sei estimates approximately **56 / 82 / 115** after ordered shared-foundation reuse; this is not a commitment. Recommended order is A → B → D → E → F → G → H, with C after B but independently parked until storage, routing, and private test-access feasibility are proven. G should be split into read-only recommendation/evidence, responsive decision UI, and purchase integration. Each phase receives its own later authorization. No phase is currently executable.
