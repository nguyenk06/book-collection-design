...
- **Confirmed validation remediation — saved unpublished:** checkpoint `80e4c61` remains retired as historical evidence. Replacement checkpoint `f15ea81`, based on exact Version 20, implements primary multi-row Bookcase, horizontal Shelf alternate, selectable informative missing positions, selected-detail cover/fallback presentation, focus restoration, and mobile Shopping discovery. It is fully validated and saved unpublished as Version 21. Bounded publication is authorized but unrun; live validation remains conditional on successful publication. See the [Version 20 remediation plan](VERSION20_CONFIRMED_FAILURE_REMEDIATION_PLAN.md).
