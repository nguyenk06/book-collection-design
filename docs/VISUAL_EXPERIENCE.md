...
- **Confirmed validation remediation — historical evidence only:** checkpoint `80e4c61` demonstrated primary multi-row Bookcase, horizontal Shelf alternate, selectable informative missing positions, selected-detail cover/fallback presentation, and focus restoration, but its editable source was not preserved and the checkpoint is retired as an executable candidate. A new candidate must follow the [Version 20 remediation plan](VERSION20_CONFIRMED_FAILURE_REMEDIATION_PLAN.md) and earn new validation.
