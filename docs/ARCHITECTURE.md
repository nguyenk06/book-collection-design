...
The surface was saved and published as Site Version 19. Gate 2 used its owner-authenticated same-origin path to observe the expected pre-upgrade baseline and retain a private structured export. Gate 3 later invoked its guarded activation exactly once. Gate 4 used the read-only verification action to independently confirm Shopping schema completion with zero foreign-key issues; a separately authorized one-time bridge export then matched Gate 2 counts, identities, Book values, ownership, copies, and cover references, with only the authorized 600-cent CYOA target-price change. Gate 4 is complete within that bridge-observable boundary, not as a D1 snapshot, R2-byte backup, restore-readiness proof, or complete backup. The surface stages status before export/upgrade, withholds controls and data from non-owners, validates private export structure before browser download, and keeps raw records out of rendered feedback. The schema-changing POST adds server-enforced same-origin, JSON content-type, dedicated action-header, and exact confirmation checks before migration logic. Client locking reduces accidental duplicate submission; server authorization and durable schema inspection remain authoritative.
