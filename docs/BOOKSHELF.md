...
Historical checkpoint `80e4c61` remains retired as an executable candidate. Replacement remediation from exact Version 20 is complete at clean checkpoint `f15ea81` and saved unpublished as Version 21. It provides `Bookcase` as the primary wrapping multi-row view, `Shelf` as an explicit horizontal alternate, selectable missing positions with canonical number/title and safe existing metadata, selected-detail cover/fallback presentation, and restored focus/scroll context. Prior focused/full/lint/build/responsive/accessibility/package evidence remains retained, but the later pre-publication focused suite failed 3/17 and stopped the release before any publication attempt. Diagnostic-only classification is authorized and prepared; it cannot change source or resume publication. Product Owner hands-on validation is still conditional on successful publication. See the [remediation plan](VERSION20_CONFIRMED_FAILURE_REMEDIATION_PLAN.md).
