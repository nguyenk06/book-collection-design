...
Gates 0–5 are complete within their documented evidence boundaries. Replacement `f15ea81` was saved and definitively published as Version 21 after the final build/package/boundary gates passed. The first publication invocation remains historically ambiguous; controlled Attempt 2 returned deployment identity and `succeeded` status. After propagation, all five authorized anonymous public markers matched. Product Owner hands-on validation, broad smoke, correction, later publication, and recovery remain separate gates.
