...
Gates 0–5 are complete within their documented evidence boundaries. Replacement `f15ea81` was saved as Version 21 and passed all local release gates. Its sole 1/1 publication invocation returned no deployment identity or success confirmation. The later identity brief could not be accepted because the supported status lookup requires that missing ID and no current-version/history metadata surface is exposed; query count remains zero. A live release exists, but exact version identity is unverified and Version 20 is only the last confirmed pre-invocation baseline. A new evidence-standard decision controls behavior-level public fingerprinting versus parking. Retry, health verification, and the Product Owner checklist remain separate closed gates.
