...
- **Queue Mode:** `ENABLED`.
- **Throttle:** `DRAIN`; Version 25 corrective planning is open, and no application writer or Site operation is active.
- **Priority decision:** Product Owner review accepted Version 24 desktop/mobile shell, navigation, header, and view presentation but failed scanner hands-on acceptance. Corrective Step 1 records the outcome and contract; Steps 2–3 require a separately authorized Kira brief. My Library aggregate semantics are deferred to Step 4, and cover loading is outside Steps 1–3.
...
- **Next owner:** `PLANNER — QUATRE` — prepare a separately gated Kira Steps 2–3 scanner diagnosis/restoration brief from [Scanner and Matching](SCANNER_AND_MATCHING.md). No implementation, Site, or release action is implied by this documentation step.
- **Usage:** The current authoritative account meter observed after the natural five-hour reset on 2026-09-07 reported 99% five-hour remaining and 68% weekly remaining; all three reset credits remain unused. The Phase A completion reading was 25% / 73%. Preserve the five-hour 10% clean-stop target and 5% hard boundary plus the separate 15% weekly floor. Capacity does not authorize release or another phase. Sites capacity is `UNKNOWN` unless a meter is actually exposed.
...
- M1, M4, M5, and M6 and M2 Gates 0–5 are complete within their recorded evidence boundaries. M3 implementation/test evidence passed and was published, but Product Owner public hands-on scanner acceptance failed. Version 25 corrective work remains open without current implementation or release authority.
...
- Phase A — IA and responsive shell is saved and publicly deployed as Version 24 at exact `4cd6eec`, with focused 60/60, full 112/112, lint 0 errors/two established warnings, build/boundary passes, and Relena `PASS`. Product Owner accepted its shell/navigation/header/view presentation. Overall corrective work remains open: scanner Steps 2–3 come first, My Library aggregate semantics are deferred to Step 4, and cover loading remains a later-phase candidate. Any remediation or later release remains separate.
...
### Scanner restoration — Version 23 evidence retained; Version 25 correction open

The Product Owner authorized the bounded Version 23 restoration. Quatre implemented from exact Version 22 because the Site-managed lifecycle prevented Kira from writing the managed checkout; Kira used no Sites capability and made no source change. Relena's read-only source/release-boundary review returned `PASS`; that review and the automated evidence did not substitute for Product Owner hands-on acceptance.

The accepted seven-path boundary added the shared scanner resolver and its focused unit test and changed only the existing Library and Shopkeeper entry-point/style/test paths. `app/api/isbn/[isbn]/route.ts`, APIs, authentication, schema, migrations, dependencies, lockfile, and hosting remain unchanged.

The Version 23 implementation/test evidence says the flow normalizes/validates; compares exact/equivalent ISBN across the whole Library; shows tracked/conflict results; uses the existing ISBN API only after no local match; exposes metadata success/empty/failure; routes to explicit Library review/add or manual fallback; and rechecks duplicates before confirmed creation. Shopkeeper is intended to use the same resolver with a simplified Library handoff, and scanning/lookup must never mutate data. Published Version 24 hands-on behavior did not demonstrate those downstream outcomes after ISBN decode/display, so the restoration is not accepted.

The exact seven-path checkpoint is `75075e1e69735ac38c5afdbb7c9b7c55c12a2647`. Focused resolver/Library/Shopkeeper/collection tests pass 57/57; the uncontended full suite passes 109/109; lint passes with zero errors and two established warnings; production build, diff/whitespace, no-unconfirmed-mutation, and protected-boundary checks pass under the bundled supported Node runtime. The implementation delta contains no API-route, authentication, schema, migration, dependency, lockfile, or hosting-file change.

The exact checkpoint was pushed to configured `main`, packaged into a validated 47-file archive, saved as Version 23, and published once to the existing public audience. Deployment returned definitive `succeeded` at the existing CYOA URL. No schema/data action, migration execution, provider redesign, dependency change, held Version 21 validation, retry, rollback, or restore occurred.
...
### Phase A Version 24 — shell presentation accepted; corrective work open
...
- **Preservation:** Keep existing data, actions, authentication/owner boundaries, scanner behavior, and route reachability intact; no schema, API/provider, persistence, or Site expansion is implied.
- **Responsive acceptance:** Representative desktop and mobile widths retain understandable navigation, readable state, safe-area behavior, keyboard/focus order, touch targets, and no unintended horizontal overflow.
- **Explicit exclusions:** Phase F's denser-spine, stored-cover detail, and compact missing-book summary decisions remain deferred. Historical `608553f` source must not be imported or treated as current validation.
- **Planning estimate (closed):** Kira estimated **9 / 13 / 18 percentage points** low/expected/high through a local Relena-ready packet, excluding Site publication. The implementation began at 39% five-hour / 75% weekly and closed at 25% / 73%, without using a reset credit.
...
- **Published-review result:** Desktop/mobile shell, navigation, header, and view behavior are accepted. My Library aggregate semantics are incorrect and deferred to Step 4. Selecting a book does not load covers and remains a later-phase candidate outside Steps 1–3. Library and Shopkeeper scanner hands-on acceptance failed because decode/display did not continue through the required shared resolution workflow.
- **Corrective Steps 2–3:** Diagnose the source/runtime handoff without production-data access, then make camera/manual entry on both surfaces use one shared resolver with whole-Library ISBN equivalence, explicit tracked/conflict/unmatched/error outcomes, metadata only after no local match, Library review/add or manual fallback, compact Shopkeeper-to-Library handoff, no scan/lookup mutation, duplicate recheck before confirmed create, and repeat-scan reset. The canonical acceptance and stop contract is [Scanner and Matching](SCANNER_AND_MATCHING.md).
- **Remaining gates:** Steps 2–3 require separate execution authority and fresh evidence. Any need for Sites/credentials, production data, deployment, schema/migration/authentication/API/provider/dependency/lockfile changes must stop for Product Owner decision. Step 4, cover work, Phase F, and later publication remain excluded and separately gated.
...
1. **Historical evidence — Phase A responsive shell:** My Library identity/navigation, contextual row, mobile header/bottom navigation, Shopkeeper presentation, collection framing, footer, and owner/public boundaries were reported at checkpoint `608553f`; that object exists only in the retained historical checkout and is not transferable current source.
2. **Historical evidence — static Phase D theme foundation:** semantic color conversion and desktop/mobile theme, focus, overflow, and sampled contrast checks were reported at `608553f`; they are not a transferable or currently validated implementation.
...
Under the superseded reserve rule used for that completed batch, the combined envelope was **14–23 percentage points** and required approximately 53% to start. This is historical execution evidence, not the current usage rule.
...
- M4 bounded Bookshelf was first published in Version 20, corrected in Version 21, and preserved through Version 24 without Phase F expansion; older Product Owner hands-on validation remains parked.
- M5 catalog export foundation is carried unchanged through Version 24. The private Gate 2 bridge export and M5 catalog export are useful but neither is a complete production backup; both exclude R2 bytes.
