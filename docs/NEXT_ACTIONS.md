...
- **Engineer state:** Sei II retired after a verified private continuity handoff and sanitized Designer report. Sei III is replacement-prepared and has an eligible superseding continuation brief, but is not initialized, verified, brief-accepted, or active.
- **Next owner:** `PRODUCT OWNER` — open the prepared Sei III replacement context and send `!init`. Only after successful read-only context verification should Sei receive `!brief`, followed by a separate `!run`.
