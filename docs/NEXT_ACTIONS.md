...
- **Queue Mode:** `ENABLED`.
- **Throttle:** `DRAIN`; the preservation/save brief is complete and no Engineer run is active.
...
- **Next owner:** `PLANNER — QUATRE` — process the independent publication and conditional live-validation decisions. Do not create or execute a release brief before the applicable decision and a fresh usage reading.
- **Usage:** Sei III recorded `!20:72` after the Version 21 save. Release execution is `WAITING FOR RESET`; the 15% floor remains mandatory.
