...
- **Queue Mode:** `ENABLED`.
- **Throttle:** `DRAIN`; the accepted preservation/save `!run` remains active through Stage 2.
...
- **Next owner:** `ENGINEER — SEI III` — continue the active `!run` through Stage 2 pre-save revalidation and the single unpublished save. The Stage 1 report is a nonterminal checkpoint; no new `!brief` or `!run` is required.
- **Usage:** Sei III recorded `!50:76` after Stage 1. Remaining Stage 2 work is estimated at 3/5/8 points and requires a 23% operational minimum including the 15% floor.
