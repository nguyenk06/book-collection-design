...
- **Queue Mode:** `ENABLED`.
- **Throttle:** `DRAIN`; the Version 21 publication brief is prepared but no Engineer run is active.
...
- **Next owner:** `ENGINEER — SEI III` — accept the prepared exact-Version-21 publication brief with `!brief`. A separate `!run` remains mandatory before any validation or Site operation.
- **Usage:** Product Owner recorded `!96:68`. The 6/9/14-point publication slice has a 29% operational minimum including the 15% floor; capacity is sufficient.
...
On 2026-09-02 Product Owner selected Decisions 1:A and 2:A. One bounded publication brief for exact saved Version 21 is prepared and awaits separate `!brief` acceptance followed by `!run`. It permits identity and collision checks, full required pre-publication revalidation, one publication attempt, bounded application-health verification, sanitized evidence, and a clean stop. It excludes correction, retry/republish, migration, production schema/data mutation, broad smoke testing, hands-on validation, rollback, restore, and destructive recovery. A concise Version 21 hands-on checklist may be prepared only after successful publication; no live-validation action is authorized before that dependency passes.
