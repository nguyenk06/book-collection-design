...
- **Queue Mode:** `ENABLED`.
- **Throttle:** `DRAIN`; Sei III's accepted continuation is resume-eligible under the superseding quarantine tool-recovery addendum, but no execution is active until a fresh `!run`.
...
- **Next owner:** `PRODUCT OWNER` — send Sei III `!run`. The recovery addendum permits supported destination-directory creation, exactly one actual reversible quarantine move, boundary verification, and remaining local validation/checkpoint work.
- **Usage:** Product Owner supplied `!89:82`. Remaining work is estimated at 4/7/12 points and requires a 27% minimum including the 15% floor.
