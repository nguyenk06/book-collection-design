...
- When asking the Product Owner for a decision, always show the available choices, the practical impact or tradeoff of each, Quatre's recommended choice and reason, and a compact response format such as `1:A; 2:A`. Do not ask for a bare approval or make the Product Owner reconstruct options from documentation. If only one safe choice exists, state it and explain why.
